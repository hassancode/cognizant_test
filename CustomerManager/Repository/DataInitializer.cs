...
namespace CustomerManager.Repository {
	internal static class DataInitializer {
		internal static void Initialize(CustomerManagerEntities context) {
			...
			for (int i = 0; i < customerNames.Length; i++) {
				...
				var cust = new Customer {
					...
					Zip = (zip + i).ToString(),
					GenderID = (int)(Model.Gender)Enum.Parse(typeof(Model.Gender), nameGenderHost[2])
				};
